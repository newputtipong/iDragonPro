    ...
    override fun onReceive(context: Context?, intent: Intent?) {
        ...
        if (isPaused) {
            ...
            val newInputData = workDataOf(
                "downloadUrl" to downloadUrl, "outputFilePath" to outputFilePath
            )
            val newWorkRequest =
                OneTimeWorkRequestBuilder<DownloadWorker>().setInputData(newInputData)
                    .addTag(taskId).build()
            WorkManager.getInstance(context!!).enqueueUniqueWork(
                taskId, ExistingWorkPolicy.REPLACE, newWorkRequest
            )
        }
